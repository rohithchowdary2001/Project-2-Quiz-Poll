...
class SubmissionController {
        ...
        try {
            ...
            if (existingSubmission) {
                if (existingSubmission.is_completed) {
                    ...
                    const timeElapsed = Math.floor((new Date() - new Date(existingSubmission.started_at)) / 60000);
                    const timeRemaining = Math.max(0, quizInfo.time_limit_minutes - timeElapsed);

                    if (timeRemaining <= 0) {
                        // Time is up, auto-submit
                        await SubmissionController.autoSubmitQuiz(existingSubmission.id, req);
    ...
    static async submitAnswer(req, res, next) {
        try {
            ...
            // Validate required fields
            ErrorHandler.validateRequired(['submissionId', 'questionId', 'selectedOptionId'], req.body);
            ...
            const timeElapsed = Math.floor((new Date() - new Date(submissionInfo.started_at)) / 60000);
            ...
            if (existingAnswer) {
                ...
                await database.update('student_answers', {
    ...
    static async completeQuiz(req, res, next) {
        try {
            ...
            await database.update('quiz_submissions', {
                is_completed: true,
            ...
            if (submissionInfo.show_results_after_submission) {
                const detailedResults = await SubmissionController.getSubmissionDetails(submissionId);
    ...
    static async getSubmissionResults(req, res, next) {
        try {
            ...
            const result = {
                ...
                percentage: Math.round((submissionInfo.total_score / submissionInfo.max_score) * 100)
            };

            // Include detailed results if quiz allows or if explicitly requested by professor/admin
            if (submissionInfo.show_results_after_submission) {
                const detailedResults = await SubmissionController.getSubmissionDetails(id);
    ...
    static async getPollResults(req, res, next) {
        try {
            ...
            if (!submission) {
                throw ErrorHandler.forbiddenError('You must complete the quiz to see poll results');
            }
    ...
    static async autoSubmitQuiz(submissionId, req) {
        try {
            const submission = await database.findById('quiz_submissions', submissionId);
            if (!submission || submission.is_completed) {
                return;
            }

            const timeElapsed = Math.floor((new Date() - new Date(submission.started_at)) / 60000);
...
// Submission routes
router.post('/start', AuthMiddleware.verifyToken, AuthMiddleware.requireStudent, ErrorHandler.asyncHandler(SubmissionController.startQuiz));
router.post('/answer', AuthMiddleware.verifyToken, AuthMiddleware.requireStudent, ErrorHandler.asyncHandler(SubmissionController.submitAnswer));
router.post('/complete', AuthMiddleware.verifyToken, AuthMiddleware.requireStudent, ErrorHandler.asyncHandler(SubmissionController.completeQuiz));
router.get('/my-submissions', AuthMiddleware.verifyToken, AuthMiddleware.requireStudent, ErrorHandler.asyncHandler(SubmissionController.getStudentSubmissions));
router.get('/:id', AuthMiddleware.verifyToken, AuthMiddleware.requireStudent, ErrorHandler.asyncHandler(SubmissionController.getSubmissionResults));
router.get('/quiz/:quizId/status', AuthMiddleware.verifyToken, AuthMiddleware.requireStudent, ErrorHandler.asyncHandler(SubmissionController.getQuizStatus));
router.get('/poll-results/:quizId', AuthMiddleware.verifyToken, AuthMiddleware.requireStudent, ErrorHandler.asyncHandler(SubmissionController.getPollResults));
