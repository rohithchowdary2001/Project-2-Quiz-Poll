 ...
 static async getAllClasses(req, res, next) {
  try {
    ...
    if (user.role === "admin") {
      ...
      `;
      // no params for admin where clause
    ...
    // Because LIMIT and OFFSET cannot be passed as parameters, inject them safely after validation
query += ` ORDER BY ${sortColumn} ${sortDirection} LIMIT ${limitNum} OFFSET ${(pageNum - 1) * limitNum}`;


    // Debug print to console
    console.log("getAllClasses SQL:", query);
    console.log("getAllClasses Params:", queryParams);
  ...
  static async createClass(req, res, next) {
    try {
      ...
      const existingClass = await database.findOne("classes", {
  ...
  static async deleteClass(req, res, next) {
    try {
      ...
      if (!classData) {
        throw ErrorHandler.notFoundError(
          "Class not found or you do not have permission to delete it"
        );
      }
...
router.get(
  ...
  ErrorHandler.asyncHandler(ClassController.getAllClasses)
);
router.get(
  "/:id",
  AuthMiddleware.verifyToken,
  ErrorHandler.asyncHandler(ClassController.getClassById)
);
router.post(
  "/",
  AuthMiddleware.verifyToken,
  AuthMiddleware.requireProfessor,
  ErrorHandler.asyncHandler(ClassController.createClass)
);
router.put(
  "/:id",
  AuthMiddleware.verifyToken,
  AuthMiddleware.requireProfessor,
  ErrorHandler.asyncHandler(ClassController.updateClass)
);
router.delete(
  "/:id",
  AuthMiddleware.verifyToken,
  AuthMiddleware.requireProfessor,
  ErrorHandler.asyncHandler(ClassController.deleteClass)
);
router.get(
  "/:id/students",
  AuthMiddleware.verifyToken,
  ErrorHandler.asyncHandler(ClassController.getClassStudents)
);
router.post(
  "/:id/students",
  AuthMiddleware.verifyToken,
  AuthMiddleware.requireProfessor,
  ErrorHandler.asyncHandler(ClassController.enrollStudent)
);
router.delete(
  "/:id/students/:studentId",
  AuthMiddleware.verifyToken,
  AuthMiddleware.requireProfessor,
  ErrorHandler.asyncHandler(ClassController.removeStudent)
);
router.get(
  "/:id/stats",
  AuthMiddleware.verifyToken,
  ErrorHandler.asyncHandler(ClassController.getClassStats)
);
